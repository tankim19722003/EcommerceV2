...
const ProductCreationForm = () => {
  ...
  const [shippingTypes, setShippingTypes] = useState([
    { type: "Tiêu chuẩn", enabled: false, cost: null, eta: "3-5 ngày" },
    { type: "Nhanh", enabled: false, cost: null, eta: "1-2 ngày" },
    { type: "Hỏa tốc", enabled: false, cost: null, eta: "Trong 24h" },
  ]);
  ...
  const validateForm = () => {
    ...
    if (!thumbnail) newErrors.thumbnail = "Vui lòng tải hình ảnh đại diện.";
    ...
    if (categoryType === "one-level") {
      ...
      if (
        oneLevelCategories.some(
          (cat) => !cat.image || !cat.value || !cat.quantity || !cat.price
        )
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      productFormData.append("sub_category_id", basicInfo.subCategoryId);
      productFormData.append("shop_id", basicInfo.shopId);
      productFormData.append("discount", basicInfo.discount || 0);
      productFormData.append("notes", "");
      productFormData.append("thumbnail", thumbnail);
      productImages.forEach((image, index) => {
        productFormData.append(`product_images[${index}]`, image);
      });

      const productResponse = await api.post(
        "http://localhost:8080/api/v1/productivity/create_product/1",
        productFormData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
      const productId = productResponse.data.product_id || 1;

      if (categoryType === "one-level") {
        const categoryFormData = new FormData();
        categoryFormData.append(
          "product_category_group_name",
          oneLevelCategoryGroup
        );
        oneLevelCategories.forEach((cat, index) => {
          categoryFormData.append(
            `product_categories[${index}][value]`,
            cat.value
          );
          categoryFormData.append(
            `product_categories[${index}][quantity]`,
            cat.quantity
          );
          categoryFormData.append(
          ...
          categoryFormData.append(
            `product_categories[${index}][image]`,
            cat.image
          );
        });
        await api.post(
          `http://localhost:8080/api/v1/product_category/add_multiple/one_level?product_id=${productId}&shop_id=${basicInfo.shopId}`,
          categoryFormData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
      } else {
        ...
        categoryFormData.append(
          ...
          twoLevelCategoryGroup
        );
        categoryFormData.append(
          "sub_product_category_group",
          twoLevelSubCategoryGroup
        );
        twoLevelCategories.forEach((cat, catIndex) => {
          categoryFormData.append(
            `product_category_two_level[${catIndex}][parent_product_category]`,
            cat.parent_product_category
          );
          categoryFormData.append(
            `product_category_two_level[${catIndex}][image]`,
            cat.image
          );
          cat.child_product_categories.forEach((child, childIndex) => {
            categoryFormData.append(
              `product_category_two_level[${catIndex}][child_product_categories][${childIndex}][name]`,
              child.name
            );
            categoryFormData.append(
              `product_category_two_level[${catIndex}][child_product_categories][${childIndex}][quantity]`,
              child.quantity
            );
            categoryFormData.append(
              `product_category_two_level[${catIndex}][child_product_categories][${childIndex}][price]`,
              child.price
            );
          });
        });
        await api.post(
          `http://localhost:8080/api/v1/product_category/add_multiple/two_level?product_id=${productId}&shop_id=${basicInfo.shopId}`,
          categoryFormData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
      }
      ...
      if (attributes.length > 0) {
        await api.post(
          `http://localhost:8080/api/v1/product_attribute_value/add_multiple?product_id=${productId}&shop_id=${basicInfo.shopId}`,
          attributes
        );
      }
      ...
      await api.post(
        "http://localhost:8080/api/v1/product_shipping/create_product_shipping_type",
        {
          ...
          shop_id: basicInfo.shopId,
          dimensions,
          shipping_types: shippingTypes.map(({ type, enabled }) => ({
            type,
            enabled,
          })),
        }
        ...
        <div className="bg-white rounded-md shadow-md border border-gray-200 p-5 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleBasicInfoChange}
                    className={`w-full px-4 py-2 border rounded-md bg-white text-gray-800 text-base leading-6 transition-all duration-150 focus:outline-none focus:border-teal-500 ${
                      errors.subCategoryId ? "border-red-600" : "border-gray-200"
                    }`}
            ...
            >
              ...
              {categoryType === "one-level" ? (
                <div className="space-y-4 mt-4">
                  <div>
                    <label className="block text-base font-medium text-gray-800 mb-1">
                      Tên Nhóm Phân Loại <span className="text-red-600">*</span>
                    </label>
                    <input
                      type="text"
                      value={oneLevelCategoryGroup}
                      onChange={(e) => setOneLevelCategoryGroup(e.target.value)}
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-200">
              <button
                type="button"
                className="px-4 py-2 border border-gray-200 text-gray-800 rounded-md hover:bg-gray-100 text-base font-medium transition-colors duration-150"
                onClick={() =>
